...
function onCompletedCardPrev(item) {
  if (item) {
    item.classList.remove('process_item-show');
    item.classList.add('process_item-hide');
  }
}

function onRemoveCardPrev(item) {
  if (item) {
    item.classList.add('process_item-show');
  }
}
...
function animateStackCards() {
  ...
  for (var i = 0; i < this.items.length; i++) {
    ...
    if (scrolling > 0) {
      ...
      if ((prevCard = this.items[i - 1])) {

        let currentCardTop = this.items[i].getBoundingClientRect().top;
        let prevCardTop = prevCard.getBoundingClientRect().top;
        let diff = currentCardTop - prevCardTop;

        if (diff <= 12 && isScrollingDown) {
          prevCard.style.opacity = opacity;
          opacity -= 0.1;
          onCompletedCardPrev(prevCard);
        }

        if (diff >= 12 && !isScrollingDown) {
          onRemoveCardPrev(prevCard);
        }
      }
